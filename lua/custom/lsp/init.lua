return { setup = function(ops)
    ops = vim.tbl_deep_extend("force", require "custom.lsp.default", ops)
    ...
    for server, options in pairs(ops.servers) do
      if options.enable == true then
        local ok, definition = pcall(require, "custom.lsp.servers." .. server)
    ...
    vim.api.nvim_create_user_command("CustomLSP", function()
      vim.ui.select(servers, { prompt = "LSP Servers" }, function(server)
        ...
        local path = string.format("%s/lua/custom/lsp/servers/%s.lua", vim.fn.stdpath "config", server)
